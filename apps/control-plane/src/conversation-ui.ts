...
function promotionControls(conversation: Conversation): string {
  ...
  if (promotion.state === "accepted" || conversation.status === "promoted")
    return `<section class="card" data-conversation-landing="delivery">${issue}${run ? `<p><a class="button" href="${escapeHtml(run.url)}">Open Roundhouse run</a></p>` : ""}</section>`;
  if (promotion.state === "rejected")
    return `<section class="card" data-conversation-landing="delivery">${issue}<p class="notice">${escapeHtml(promotion.errorCode ?? "GitHub intake rejected this promotion")}. This conversation has not been marked delivered.</p></section>`;
  if (promotion.state === "awaiting_intake")
    return `<section class="card" data-conversation-landing="delivery">${issue}<p class="muted">The issue and start comment exist. This conversation will close only after the normal GitHub webhook authorizes the actor and records the run.</p></section>`;
  return `<section class="card" data-conversation-landing="delivery">${issue}<p class="muted">This operation is durable and reconciles existing GitHub writes before retrying.</p></section>`;
}
...
function messageHtml(message: Conversation["messages"][number]): string {
  return `<article class="message ${message.role}" data-message-id="${escapeHtml(message.id)}" data-version="${escapeHtml(messageVersion(message))}"><div class="meta">${message.role === "user" ? escapeHtml(message.actorLogin) : "Roundhouse"}</div><div class="message-body">${renderMarkdown(message.body)}</div></article>`;
}
...
export function renderConversation(
  ...
  const title = conversation.title?.trim() || "New conversation";
  const status = conversationStatus(conversation);
  const script = `<script defer src="/assets/conversation-poll.js" data-state-url="/conversations/${encodeURIComponent(conversation.id)}/state" data-polling="${state.polling}"></script>`;
  const rendered = page(
    "Conversation",
    user,
    `<header class="conversation-header"><div><a class="meta" href="/conversations">Conversations</a><h1>${escapeHtml(title)}</h1><div class="conversation-summary"><span>${escapeHtml(conversation.repository.name)}</span><span class="status ${status.tone}">${escapeHtml(status.label)}</span><span class="readonly">Read only</span></div></div></header><p class="muted conversation-secondary">Repository context is pinned to <code>${escapeHtml(conversation.sourceCommit.slice(0, 12))}</code> · ${escapeHtml(conversation.context.model.id)} / ${escapeHtml(conversation.context.model.reasoning)}. The assistant can read this public snapshot and research the public web, but cannot modify anything.</p>${notice ? `<div class="notice">${escapeHtml(notice)}</div>` : ""}<div id="conversation-status" data-version="${escapeHtml(state.status.version)}" data-status-key="${escapeHtml(state.status.key)}">${state.status.html}</div><section id="conversation-messages">${state.messages.map((message) => message.html).join("")}</section><section id="conversation-controls" data-version="${escapeHtml(state.controls.version)}">${state.controls.html}</section><button id="conversation-new-response" class="new-response" type="button" hidden>New response ↓</button><div id="conversation-live-status" class="sr-only" role="status" aria-live="polite"></div>`,
    script,
  );
  console.log(
    JSON.stringify({
