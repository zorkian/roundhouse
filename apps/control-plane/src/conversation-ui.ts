...
export function renderConversation(
  ...
  const rendered = page(
    "Conversation",
    user,
    `<header class="conversation-header"><div><a class="meta" href="/conversations">Conversations</a><h1>${escapeHtml(title)}</h1><div class="conversation-summary"><span>${escapeHtml(conversation.repository.name)}</span><span class="readonly">Read only</span></div></div></header><p class="muted conversation-secondary">Repository context is pinned to <a href="https://github.com/${escapeHtml(conversation.repository.name)}/commit/${escapeHtml(conversation.sourceCommit)}"><code>${escapeHtml(conversation.sourceCommit.slice(0, 12))}</code></a> · ${escapeHtml(conversation.context.model.id)} / ${escapeHtml(conversation.context.model.reasoning)}. The assistant can read this public snapshot and research the public web, but cannot modify anything.</p>${notice ? `<div class="notice">${escapeHtml(notice)}</div>` : ""}<div id="conversation-status" data-version="${escapeHtml(state.status.version)}" data-status-key="${escapeHtml(state.status.key)}">${state.status.html}</div><section id="conversation-messages">${state.messages.map((message) => message.html).join("")}</section><section id="conversation-controls" data-version="${escapeHtml(state.controls.version)}">${state.controls.html}</section><button id="conversation-new-response" class="new-response" type="button" hidden>New response ↓</button><div id="conversation-live-status" class="sr-only" role="status" aria-live="polite"></div>`,
    script,
  );
  console.log(
    JSON.stringify({
