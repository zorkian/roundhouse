...
<dl><dt>Status</dt><dd><span class="status ${escapeHtml(run.status)}">${escapeHtml(statusLabels[run.status])}</span></dd><dt>Current stage</dt><dd>${escapeHtml(currentStage)}</dd><dt>Elapsed</dt><dd>${escapeHtml(elapsed(details.createdAt, details.updatedAt))}</dd><dt>Total usage</dt><dd>${usageDisplay(usage)}</dd><dt>Source issue</dt><dd>${link(run.issue?.url, `Issue #${run.issueNumber}`)}</dd>${prRow}<dt>Created</dt><dd>${escapeHtml(new Date(details.createdAt).toISOString())}</dd><dt>Updated</dt><dd>${escapeHtml(new Date(details.updatedAt).toISOString())}</dd></dl>
${outcomeSection}<section><h2>Attempt history</h2>${rows || '<p class="muted">No attempts recorded.</p>'}</section><section><h2>Diagnostics</h2>${runDiagnostics}${reviewWorkflowEvidence(details)}${boundaryWorkflowEvidence(details)}${profileSection}</section></body></html>`;
}
