...
function reviewWorkflowEvidence(details: RunDetails): string {
  ...
  if (!events.length) return "";
  return `<section><h2>Review workflow evidence</h2>${events
    .map(
      (event) =>
...
function boundaryWorkflowEvidence(details: RunDetails): string {
  ...
  if (!events.length) return "";
  return `<section><h2>Human and external boundary evidence</h2>${events
...
export function renderRunDetails(details: RunDetails): string {
  ...
  const profileSection = run.profile
    ? `<section><h2>Repository profile</h2><dl><dt>Source path</dt><dd><code>${escapeHtml(run.profile.sourcePath)}</code></dd><dt>Source commit</dt><dd><code>${escapeHtml(run.profile.sourceCommit)}</code></dd><dt>Schema version</dt><dd>${escapeHtml(run.profile.version)}</dd><dt>Profile hash</dt><dd><code>${escapeHtml(run.profile.hash)}</code></dd><dt>Allowed paths</dt><dd>${value(run.profile.paths.allowed)}</dd><dt>Protected paths</dt><dd>${value(run.profile.paths.protected)}</dd><dt>Merge policy</dt><dd>${value(run.profile.merge)}</dd><dt>Operators</dt><dd>${value(run.profile.permissions?.operators)}</dd><dt>Development environment</dt><dd>${value(run.profile.developmentEnvironment)}</dd><dt>Project instructions</dt><dd>${value(run.profile.instructions?.project)}</dd><dt>Stages</dt><dd>${value(run.profile.stages)}</dd><dt>Reviewers</dt><dd>${value(run.profile.reviewers)}</dd><dt>Validation</dt><dd>${value(run.profile.validation)}</dd></dl></section>`
...
${reviewWorkflowEvidence(details)}${boundaryWorkflowEvidence(details)}<section><h2>Attempt history</h2>${rows || '<p class="muted">No attempts recorded.</p>'}</section>${profileSection}</body></html>`;
}
