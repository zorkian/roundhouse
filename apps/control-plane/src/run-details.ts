...
<h4>Model usage</h4><p>${escapeHtml(formatUsage(usage.filter((item) => item.attemptId === attempt.id)))}</p><p>${escapeHtml([...new Set(usage.filter((item) => item.attemptId === attempt.id).map((item) => item.model))].join(", ") || "Model unavailable")}</p><h4>Model routing</h4>${value(attempt.routing)}<h4>Result</h4>${value(attempt.result)}</article>`,
    )
    .join("");
  ...
  const sections = sectionData
    .map(([heading, content], index) => {
      ...
      const attemptIds = attempts
        .filter((attempt) => attempt.stage === stages[index])
        .map((attempt) => attempt.id);
      return `<section><h2>${heading}</h2><p>${escapeHtml(formatUsage(usage.filter((item) => attemptIds.includes(item.attemptId))))}</p>${heading === "CI checks" ? ciResult(content) : value(content)}</section>`;
    })
    .join("");
