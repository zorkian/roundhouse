...
import { runStatusTone, statusPillStyles } from "./status-ui.js";
...
function renderRun(summary: RunSummary): string {
  ...
  const label = closedBeforeCompletion
    ? "Closed on GitHub"
    : labels[run.status];
  const statusTone = closedBeforeCompletion
    ? "failed"
  ...
  <div class="run-meta"><span class="status ${statusTone}">${label}</span><span>${escapeHtml(formatUsage(summary.usage ?? []))}</span><time datetime="${new Date(summary.updatedAt).toISOString()}">Updated ${escapeHtml(new Date(summary.updatedAt).toLocaleString("en-US", { timeZone: "UTC", dateStyle: "medium", timeStyle: "short" }))} UTC</time>${github}</div>
</article>`;
}
...
export function renderDashboard(
  ...
  const attention = runs.filter(
    ({ run, githubIssueState }) =>
      githubIssueState === "open" && ["waiting", "failed"].includes(run.status),
  );
  const active = runs.filter(
    ({ run, githubIssueState }) =>
      githubIssueState === "open" && run.status === "active",
  );
  ...
  const repositories = [...new Set(runs.map(({ run }) => run.repository))]
    .sort()
    .map((repository) => {
      const [owner, name] = repository.split("/");
      return `<a href="/repositories/${encodeURIComponent(owner ?? "")}/${encodeURIComponent(name ?? "")}/workflow">${escapeHtml(repository)} workflow</a>`;
    })
    .join(" · ");
...
<p class="refresh">${runs.length ? "Showing the 50 most recently updated runs you can access." : "No enrolled repositories you can access have runs yet."} This page refreshes every 10 seconds.</p></main></body></html>`;
}
