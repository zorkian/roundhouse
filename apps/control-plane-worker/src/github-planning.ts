...
export async function listPlanningJobs(
  ...
  const rows = await env.DB.prepare(
    "SELECT job_id FROM github_planning_jobs WHERE roundhouse_environment = ? AND repository_full_name = ? AND status IN ('queued', 'running', 'retrying') ORDER BY updated_at DESC LIMIT ?",
  )
    .bind(
      binding.roundhouseEnvironment,
      binding.repositoryFullName,
