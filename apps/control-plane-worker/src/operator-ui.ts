...
function renderPlan(v){const p=v.plan.plan;const d=v.plan;const can=d.status==='proposed';const model=(p.planningEvidence||[]).find(x=>x.startsWith('Planning model:'))||'—';return card('Immutable issue plan',rows([['plan',esc(p.planId)],['issue',ext('https://github.com/zorkian/roundhouse/issues/'+p.issueNumber,'#'+p.issueNumber)],['status','<span class="'+esc(d.status)+'">'+esc(d.status)+'</span>'],['revision',esc(d.revision)+' · '+esc(d.updatedAt||p.createdAt)],['base',commit(p.baseCommit)],['model',esc(model)],['SHA-256','<code>'+esc(p.planSha256)+'</code>'],['evidence','<a href="/v1/plans/'+encodeURIComponent(p.planId)+'/evidence">'+esc(d.evidence.objectKey)+'</a>'],['evidence SHA-256','<code>'+esc(d.evidence.sha256)+'</code>'],['approved by',actor(d.approvedBy)],['run',d.runId?link('runs',d.runId):'—']]))+card('Qualification detail','<pre>'+esc(planDetail(p))+'</pre>')+(can?card('Action','<button id="approve-plan">Approve exact plan and start</button><div class="notice" id="notice"></div>'):'')}
function duration(p){const end=p.completedAt?new Date(p.completedAt).getTime():Date.now();const start=new Date(p.startedAt).getTime();return Number.isFinite(start)&&Number.isFinite(end)?Math.max(0,Math.round((end-start)/1000))+'s':'—'}
...
function agentOutputPanel(url){return '<h3>Live agent output</h3><div class="agent-output-status muted" data-agent-output-status="'+esc(url)+'">Waiting for bounded output…</div><pre class="agent-output" data-agent-output-url="'+esc(url)+'">Waiting for agent output…</pre>'}
...
function renderRun(r){const actions=(r.state==='failed'?'<button id="retry-run">Retry exact revision</button>':'')+(!['completed','cancelled','failed'].includes(r.state)?'<button class="danger" id="cancel-run">Cancel exact revision</button>':'');const plan=r.planning?'<a href="/plans/'+encodeURIComponent(r.planning.planId)+'">approved plan</a>':'—';const workflow=(r.workflows||[]).at(-1);const workflowValue=workflow?'<span class="'+esc(workflow.status)+'">'+esc(workflow.status)+'</span> · <code>'+esc(workflow.workflowInstanceId)+'</code>':'—';const model=r.implementation?.requestedModel?esc(r.implementation.requestedModel)+' · '+esc(r.implementation.requestedEffort):'—';const metadata=rows([['task',esc(r.subject||r.taskId)],['issue',r.source?ext(r.source.issueUrl,'#'+r.source.issueNumber):'—'],['state','<span class="'+esc(r.state)+'">'+esc(r.state)+'</span>'],['revision',esc(r.revision)+' · '+esc((r.events||[]).length)+' transitions'],['durable execution',workflowValue],['model',model],['plan',plan],['approved by',actor(r.approval?.approver)],['publication',r.publication?.pullRequestUrl?ext(r.publication.pullRequestUrl,'pull request'):'—']]);return card('Run',metadata+(actions?'<div class="actions">'+actions+'<div class="notice" id="notice"></div></div>':''))+card('Timeline',timelineRows(r))}
function renderReview(r){const findings=r.execution?.result?.findings||[];const disposition=new Map((r.dispositions||[]).map(x=>[x.findingId,x]));const workflow=(r.workflows||[]).at(-1);const workflowValue=workflow?'<span class="'+esc(workflow.status)+'">'+esc(workflow.status)+'</span> · <code>'+esc(workflow.workflowInstanceId)+'</code>':'—';const prSource=r.request.sourceKind==='pull_request';const attemptId=r.activeAttemptId||r.request.attemptId;const output=attemptId?card('Agent status',agentOutputPanel('/v1/reviews/'+encodeURIComponent(r.request.reviewId)+'/agent-output/'+encodeURIComponent(attemptId))):'';const model=r.execution?.result?.model?esc(r.execution.result.model)+' · '+esc(r.execution.result.requestedEffort||'legacy default'):'—';return card('Independent review',rows([['review',esc(r.request.reviewId)],['status','<span class="'+esc(r.status)+'">'+esc(r.status)+'</span>'],['durable execution',workflowValue],['model',model],['cycle',esc(r.request.cycle)+' of 2'],['source',prSource?'pull request':'implementation run'],['issue',r.request.issueUrl?ext(r.request.issueUrl,'#'+r.request.issueNumber):'—'],['pull request',ext(r.request.pullRequestUrl,'#'+r.request.pullRequestNumber)],['base',commit(r.request.baseCommit)],['reviewed head',commit(r.request.headCommit)],['patch','<code>'+esc(r.request.patchSha256)+'</code>'],['source run',prSource?'—':link('runs',r.request.runId)],['remediation run',r.remediationRunId?link('runs',r.remediationRunId):'—'],['evidence',r.execution?'<a href="/v1/reviews/'+encodeURIComponent(r.request.reviewId)+'/evidence">'+esc(r.execution.evidence.objectKey)+'</a>':'—']]))+output+card('Findings',findings.length?findings.map(f=>{const d=disposition.get(f.findingId);return rows([['severity',esc(f.severity)],['location',esc(f.path+(f.line?':'+f.line:''))],['finding',esc(f.title)],['disposition',esc(d?.disposition||'—')],['rationale',esc(f.rationale)],['recommendation',esc(f.recommendation)]])}).join('<br>'):'<span class="muted">No findings</span>')+card('Revision history',(r.events||[]).map(e=>rows([['revision',esc(e.sequence)],['event',esc(e.type)],['at',esc(e.occurredAt)]])).join('<br>'))}
...
const agentTails=new Map;
function renderAgentTail(element,tail){const lines=tail.lines.map(line=>(line.stream==='stderr'?'[stderr] ':line.stream==='system'?'[system] ':'')+line.text);element.textContent=lines.join('\\n')||'Waiting for agent output…';element.scrollTop=element.scrollHeight;const omitted=tail.truncated?' · earlier output omitted':'';const statuses=document.querySelectorAll?.('[data-agent-output-status]')||[];for(const status of statuses)if(status.dataset.agentOutputStatus===element.dataset.agentOutputUrl)status.textContent=(tail.terminal?(tail.status==='unavailable'?'Output unavailable · polling stopped':tail.status+' · polling stopped'):tail.status==='unavailable'?'Output temporarily unavailable · retrying':'Live · '+tail.lines.length+' bounded lines')+omitted}
async function pollAgentOutput(element){const url=element.dataset.agentOutputUrl;if(!url)return;let tail=agentTails.get(url);if(!tail){tail={cursor:undefined,lines:[],terminal:false,status:'running',unavailableCount:0,truncated:false};agentTails.set(url,tail)}renderAgentTail(element,tail);if(tail.terminal)return;try{const value=await api(url+(tail.cursor===undefined?'':'?cursor='+encodeURIComponent(tail.cursor)));if(value.truncated){tail.lines=[];tail.truncated=true}const seen=new Set(tail.lines.map(line=>line.cursor));for(const line of value.lines||[])if(!seen.has(line.cursor)){tail.lines.push(line);seen.add(line.cursor)}if(tail.lines.length>500)tail.lines.splice(0,tail.lines.length-500);tail.cursor=value.nextCursor;tail.status=value.status;tail.unavailableCount=value.status==='unavailable'?tail.unavailableCount+1:0;tail.terminal=['completed','failed'].includes(value.status)||(value.status==='unavailable'&&tail.unavailableCount>=3);renderAgentTail(element,tail)}catch{tail.status='unavailable';tail.unavailableCount+=1;tail.terminal=tail.unavailableCount>=3;renderAgentTail(element,tail)}}
async function pollAgentOutputs(){const elements=document.querySelectorAll?.('[data-agent-output-url]')||[];await Promise.all([...elements].map(pollAgentOutput))}
let current;async function load(){try{const openPanels=document.querySelectorAll?[...document.querySelectorAll('[data-preserve-open][open]')].map(x=>x.id):[];const path=page.kind==='dashboard'?'/v1/dashboard':page.kind==='issue'?'/v1/repositories/'+page.id:page.kind==='plan'?'/v1/plans/'+encodeURIComponent(page.id):page.kind==='review'?'/v1/reviews/'+encodeURIComponent(page.id):'/v1/runs/'+encodeURIComponent(page.id);current=await api(path);if(page.kind==='run'&&current.implementation)current.implementationReview=await api('/v1/runs/'+encodeURIComponent(page.id)+'/implementation');document.getElementById('app').innerHTML=page.kind==='dashboard'?renderDashboard(current):page.kind==='issue'?renderIssue(current):page.kind==='plan'?renderPlan(current):page.kind==='review'?renderReview(current):renderRun(current);for(const id of openPanels)document.getElementById(id)?.setAttribute('open','');bindActions();await pollAgentOutputs()}catch(e){document.getElementById('app').innerHTML=card('Error',esc(e.message))}}
