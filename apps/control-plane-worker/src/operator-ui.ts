...
<title>${title} · Roundhouse</title><style>
...
function renderDashboard(v){return '<div class="grid">'+card('Plans',v.plans.length?v.plans.map(p=>rows([['plan',link('plans',p.plan.planId)],['issue','#'+p.plan.issueNumber],['status','<span class="'+esc(p.status)+'">'+esc(p.status)+'</span>'],['updated',esc(p.updatedAt)]])).join('<br>'):'<span class="muted">No plans yet</span>')+card('Runs',v.runs.length?v.runs.map(r=>rows([['run',link('runs',r.runId)],['subject',esc(r.task.subject)],['state','<span class="'+esc(r.state)+'">'+esc(r.state)+'</span>'],['revision',esc(r.revision)]])).join('<br>'):'<span class="muted">No runs yet</span>')+'</div>'}
function renderPlan(v){const p=v.plan.plan;const d=v.plan;const can=d.status==='proposed';return card('Immutable issue plan',rows([['plan',esc(p.planId)],['issue','#'+esc(p.issueNumber)],['status','<span class="'+esc(d.status)+'">'+esc(d.status)+'</span>'],['revision',esc(d.revision)],['base','<code>'+esc(p.baseCommit)+'</code>'],['SHA-256','<code>'+esc(p.planSha256)+'</code>'],['evidence','<code>'+esc(d.evidenceObjectKey)+'</code>'],['evidence SHA-256','<code>'+esc(d.evidenceSha256)+'</code>'],['approved by',esc(d.approvedBy||'—')],['run',d.runId?link('runs',d.runId):'—']]))+card('Exact scope','<pre>'+esc(p.status==='proposed'?p.exactPaths.join('\\n'):p.findings.map(x=>x.code+': '+x.message).join('\\n'))+'</pre>')+(can?card('Action','<button onclick="approvePlan()">Approve exact plan and start</button><div class="notice" id="notice"></div>'):'')}
function renderRun(r){const attempts=r.attempts||[];const evidence=r.evidence||[];const actions=(r.state==='failed'?'<button onclick="retryRun()">Retry exact revision</button>':'')+(!['completed','cancelled','failed'].includes(r.state)?'<button class="danger" onclick="cancelRun()">Cancel exact revision</button>':'');return card('Run',rows([['run',esc(r.runId)],['task',esc(r.task.subject)],['state','<span class="'+esc(r.state)+'">'+esc(r.state)+'</span>'],['revision',esc(r.revision)],['base','<code>'+esc(r.task.baseCommit)+'</code>'],['plan',r.task.planning?link('plans',r.task.planning.planId):'—'],['patch','<code>'+esc(r.implementation?.patchSha256||'—')+'</code>'],['publication',r.publication?.pullRequestUrl?'<a href="'+esc(r.publication.pullRequestUrl)+'">pull request</a>':'—']]))+card('Attempts',attempts.length?attempts.map(a=>rows([['stage',esc(a.stage)],['number',esc(a.number)],['status','<span class="'+esc(a.status)+'">'+esc(a.status)+'</span>'],['classification',esc(a.classification||'—')],['started',esc(a.startedAt||'—')],['completed',esc(a.completedAt||'—')]])).join('<br>'):'<span class="muted">No attempts yet</span>')+card('Evidence',evidence.length?evidence.map(e=>rows([['object','<code>'+esc(e.objectKey)+'</code>'],['SHA-256','<code>'+esc(e.sha256)+'</code>'],['bytes',esc(e.size)]])).join('<br>'):'<span class="muted">No evidence yet</span>')+(actions?card('Actions',actions+'<div class="notice" id="notice"></div>'):'')}
let current;async function load(){try{current=await api(page.kind==='dashboard'?'/v1/dashboard':page.kind==='plan'?'/v1/plans/'+encodeURIComponent(page.id):'/v1/runs/'+encodeURIComponent(page.id));document.getElementById('app').innerHTML=page.kind==='dashboard'?renderDashboard(current):page.kind==='plan'?renderPlan(current):renderRun(current)}catch(e){document.getElementById('app').innerHTML=card('Error',esc(e.message))}}
...
function retryRun(){return runAction('retry')}
function cancelRun(){return runAction('cancel')}
